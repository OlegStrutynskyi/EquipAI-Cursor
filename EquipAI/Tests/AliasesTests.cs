...
public class AliasesTests : BaseTest
{
    ...
    public async Task T12_Aliases_Add_Project()
    {
        ...
        var projects = await projectsPage.GetAllCodesAndNamesAsync();
        var expectedOptions = projects
            .Select(project => $"{project.Code} — {project.Name}")
            .ToList();
        ...
        var projectOptions = await addAliasPage.GetTelemetryProjectOptionsAsync();
        projectOptions.Should().Contain(expectedOptions);
    }
    ...
    public async Task T18_Aliases_Add_Success_Project()
    {
        ...
        try
        {
            ...
            await addAliasPage.FillAliasTextAsync(aliasText);
            await addAliasPage.SelectTargetKindAsync("Project");
            var expectedResolvesTo = await addAliasPage.SelectAnyTelemetryProjectAsync();
            await addAliasPage.FillAliasTextAsync(aliasText);
            aliasesPage = await addAliasPage.CreateAliasAsync();
            ...
            var gridRow = await aliasesPage.GetProjectAliasGridRowAsync(aliasText);
            gridRow.Should().NotBeNull();
            gridRow!.AliasText.Should().Be(aliasText);
            gridRow.ResolvesTo.Should().Be(expectedResolvesTo);
        }
    ...
    public async Task T30_Aliases_Edit_Project_DefaultView()
    {
        ...
        var editAliasPage = await OpenEditAliasProjectPageAsync();

        (await editAliasPage.GetPageTitleAsync()).Should().Be(expectedPageTitle);
        (await editAliasPage.GetSubtitleAsync()).Should().Be(Config.SetupAliasProject1);
        (await editAliasPage.GetAliasTextAsync()).Should().Be(Config.SetupAliasProject1);
        (await editAliasPage.GetTargetKindAsync()).Should().Be(expectedTargetKind);
        (await editAliasPage.GetTelemetryProjectAsync()).Should().Match(value =>
            value.Equals(Config.SetupProjectName1, StringComparison.Ordinal)
            || value.EndsWith(" — " + Config.SetupProjectName1, StringComparison.Ordinal)
            || value.EndsWith(" - " + Config.SetupProjectName1, StringComparison.Ordinal)
            || value.Contains(Config.SetupProjectName1, StringComparison.Ordinal));
        (await editAliasPage.IsBackToAliasesBtnVisibleAsync()).Should().BeTrue();
        (await editAliasPage.IsSaveAliasBtnVisibleAsync()).Should().BeTrue();
        (await editAliasPage.IsCancelBtnVisibleAsync()).Should().BeTrue();
    }
    ...
    public async Task T31_Aliases_Edit_Project_Success()
    {
        ...
        try
        {
            aliasId = await SqlHelper.GetReferenceAliasIdByAliasTextAsync(Config.SetupAliasProject1);

            var editAliasPage = await OpenEditAliasProjectPageAsync();
            await editAliasPage.FillAliasTextAsync(updatedAliasText);
            var expectedResolvesTo = await editAliasPage.SelectDifferentTelemetryProjectAsync(
            ...
            var gridRow = await aliasesPage.GetProjectAliasGridRowAsync(updatedAliasText);
            gridRow.Should().NotBeNull();
            gridRow!.AliasText.Should().Be(updatedAliasText);
            gridRow.ResolvesTo.Should().Be(expectedResolvesTo);
            (await aliasesPage.IsAliasTextInGridAsync(Config.SetupAliasProject1)).Should().BeFalse();
        }
        finally
        {
            if (aliasId is not null)
                await SqlHelper.RestoreReferenceAliasProjectAsync(
                    aliasId,
                    Config.SetupAliasProject1,
                    Config.SetupProjectName1);
        }
    }
